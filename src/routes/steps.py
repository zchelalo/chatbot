...
def get_step(id: int) -> StepSchema:
  db = Session()
...
def create_step(step: StepSchema) -> StepSchema:
  if step.id:
    step.id = None
  db = Session()
...
def update_step(id: int, step_update: StepUpdateSchema) -> StepSchema:
  db = Session()
